...
import { EditPhotos } from '../JobSeekerProfileScreen/Components/forms/edit.photos';
...
export class PhotoBox extends Component<any, any> {
    ...
    public renderImages() {
        if (this.props.photos && this.props.photos.length) {
            return <div className={'display-flex flex-wrap'}>
                {this.addMore()}
                {(this.props.photos.slice(0, 8)).map((photo: string, index: number) => <div className={'mb-6 mx-4'} key={`${photo}-${index}`}>{this.renderImage(photo, index === 7)}</div>)}
            </div>;
        }
    ...
    public render() {
        return <BoxWrapper
            content={<EditPhotos meta={this.props.photos}/>}
