...
const Posting = ({ title, description, location, salary, className }: Post) => {
    return <div className={`display-flex flex-column ${className}`}>
        ...
        <NavLink href={'/'}><p className={'text-color--light-1 fs-12 fw--400 hover-secondary'}>See more...</p></NavLink>
    </div>;
};

export const FreshPostings = ({ data }: { data: Post[] }) => {

    return <div className={'display-flex flex-column mt-170 justify-content-center align-items-center'}>
