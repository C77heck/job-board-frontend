...
const Favourites: NextPage = (props: any) => {
    ...
    useEffect(() => {
        if (isLoggedIn && token && role === 'job-seeker') {
            (async () => getJobAds())();
        }
    ...
    const getJobAds = async () => {
        try {
            if (!isLoggedIn || role !== 'job-seeker' || !token) {
                return;
            }

            const query = QueryManager.decodeBase64(window.location.search);
            const filters = extractFilters(query);
            const response = await client('/users/job-seeker/favourites', 'GET', {}, { pagination, filters });
